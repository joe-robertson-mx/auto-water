...
class ExplorerHat(BaseSensor):
    ...
    def read_moisture(self) -> None:
        ...
        obj: Schedule = Schedule(datetime=datetime.now(), water_level=water_level)
        with Session(self.engine) as session:
            session.add(obj)
            session.commit()

    def water(self, water_time: float = 3.0) -> None:
        ...
        watered_on: Watered = Watered(date_watered=datetime.now())
        with Session(self.engine) as session:
...
class TestSensor(BaseSensor):
    ...
    def read_moisture(self) -> None:
        ...
        water_level: Schedule = Schedule(datetime=datetime.now(), water_level=random.randint(0, 100))
        with Session(self.engine) as session:
